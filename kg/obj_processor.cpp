...
void OBJ_Processor::add_object(vertex *vertexes, int *indicies,int vcount, int icount, QRgb color, vertex opos, vertex orot, vertex oscale, bool clean) {
    ...
    //std::memcpy(this->indicies+icount,indicies,icount*sizeof(int));
    int pos = 0;
...
void OBJ_Processor::rotate(int id, float x, float y, float z) {
    objects[id].rot.x += x;
    /*
    if (objects[id].rot.x > 3.14) {
        objects[id].rot.x = 0;
    }
    if (objects[id].rot.x < 0) {
        objects[id].rot.x = 3.14;
    }
    */

    objects[id].rot.y += y;
    /*
    if (objects[id].rot.y > 3.14) {
        objects[id].rot.y = 0;
    }
    if (objects[id].rot.y < 0) {
        objects[id].rot.y = 3.14;
    }
    */

    objects[id].rot.z += z;
    /*
    if (objects[id].rot.z > 3.14) {
        objects[id].rot.z = 0;
    }
    if (objects[id].rot.z < 0) {
        objects[id].rot.z = 3.14;
    }
    */
}
...
// ОКРУГЛЕНИЕ ДО ЦЕЛОГО, ФУНКЦИЯ ТОЛЬКО ДЛЯ ЗАРИСОВКИ
...
// CCW triangle! Только по против часовой стрелки
...
// модифицированный алгоритм быстрой отрисовки линии с подержкой интерполяции Z координаты
        ...
        if ( long_len > 0 ) {
            ...
            for (int  j = 0x8000 + ( x << 16); y <= long_len; ++y) {
                if ( y >= 0 && y < 600 && (j>>16) >=0 && (j>>16) < 800 && check_zbuffer(j >> 16, y, z)) {
                    ...
                    //image_line[j >> 8] = QColor::fromHsv( (z*z > 359) ? 359 : z*z, 100, 100).rgb();
    ...
    if ( long_len > 0 ) {
        ...
        for (int j = 0x8000 + ( y << 16 ); x <= long_len; ++x) {
            if (x >= 0 && x < 800 && (j >> 16) >=0 && ( j >> 16) < 600 && check_zbuffer(x, j >> 16, z)) {
                ...
                //image_line[x] = QColor::fromHsv( (z*z > 359) ? 359 : z*z, 100, 100).rgb();
                image_line[x] = color;
...
void OBJ_Processor::draw_filled_triangle(const int & vertex_id, const QRgb &edge_color) {
    ...
    // эту часть все равно хорошо оптимизирует компилятор :<
    ...
    // эта часть выглядит криво, но на самом деле она написана так
    // потому что в таком виде компилятор оптимизирует лучше любого человека
...
void OBJ_Processor::calculate_vertex_sreen_projection() {
    ...
    for (int objid = 0; objid < obj_count; ++objid) {
        ...
        // заранее вычисляются синусы и косинусы для всех точек объекта
        ...
        for (int vertexid = calc_obj.vertex_start; vertexid < calc_obj.vertex_end; ++vertexid) {
            ...
            shadow_vertexes[vertexid].y = light_plane_y; // plane ZX (y = shadow plane y)
...
void OBJ_Processor::paintEvent(QPaintEvent *event) {
    ...
    /*
    // отображение карты глубин на экран
    // ОЧЕНЬ МЕДЛЕННО
    for (int y = 0; y < 600; ++y) {

        QRgb * scan = reinterpret_cast<QRgb*>(draw_image->scanLine(y));
        double zvalue;
        for (int x = 0; x < 800; ++x) {
            zvalue = zbuffer[x + y * 800];
            zvalue *= 15;
            if (zvalue < 0)   zvalue = 0;
            if (zvalue > 359) zvalue = 359;
            scan[x] = QColor::fromHsv( zvalue, 100, 100).rgb();
        }
    }*/
